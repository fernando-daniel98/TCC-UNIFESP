...
\institute[UNIFESP]{%
  ...
  Bacharelado em Engenharia de Computação \\[3mm]
  {\small Seminário de Pesquisa -- Dezembro/2025}
}
...
\begin{frame}{Contextualização}
  \begin{itemize}
    \item \textbf{Cenário global:}
      \begin{itemize}
        \item Crescente demanda por alimentos
        \item Pressão sobre recursos hídricos
        \item Necessidade de agricultura sustentável
      \end{itemize}
    
    \vspace{0.3cm}
    
    \item \textbf{Objetivos de Desenvolvimento Sustentável (ODS):}
      \begin{itemize}
        \item \textbf{ODS 2:} Fome Zero e Agricultura Sustentável
        \item \textbf{ODS 6:} Água Potável e Saneamento
        \item \textbf{ODS 12:} Consumo e Produção Responsáveis
      \end{itemize}
    
    \vspace{0.3cm}
    
    \item \textbf{Desafio:} Como usar a água de forma mais eficiente na agricultura?
  \end{itemize}
\end{frame}
...
\begin{frame}{Motivação}
  \begin{columns}
    ...
    \begin{itemize}
      \item Escassez de água em diversas regiões
      \item Desperdício na irrigação tradicional
      \item Falta de informação em tempo real
      \item Adoção fragmentada de tecnologias
      \item Dificuldade de integração de dados
    \end{itemize}
    
    \column{0.5\textwidth}
    \textbf{Oportunidades Tecnológicas:}
    \begin{itemize}
      \item Internet das Coisas (IoT)
      \item Sensoriamento remoto orbital
      \item Machine Learning
      \item Dados abertos (Sentinel, Landsat)
      \item Agricultura de precisão 4.0
    \end{itemize}
  \end{columns}
  ...
  \begin{alertblock}{Questão Central}
    Como integrar dados de sensores em campo com imagens de satélite para otimizar o manejo da irrigação?
  \end{alertblock}
\end{frame}
...
\begin{frame}{Definição do Problema}
  \begin{block}{Problema}
    A gestão hídrica na agricultura ainda ocorre de forma fragmentada, sem aproveitar plenamente o potencial da integração entre:
    \begin{itemize}
      \item Medições de alta frequência temporal (sensores IoT)
      \item Cobertura espacial ampla (imagens de satélite)
